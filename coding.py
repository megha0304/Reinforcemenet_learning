...
GAMMA = 1
LAMBDA = 0.9
F_BASE = 4 # basis vector will have length F_BASE * stateDimension


'''
Environment class
	- needs to be updated for a new environment
	- all environment class should have functions getState, transition, isTS, newEpisode
	- use specific function name in the main function
	- "mountainCar"
		- implemented Mountain car environment, state dimension 2, 3 actions
		- maxEps can be edited in init
		- learning rate can be modified in transition
'''
...
class EnvironmentMC:
	...
	# require : checks if terminal state reached
	def isTS(self):
